...
def get_teacher_jac_dense(data, forward, num_perts_per_row,num_hessian_rows, force_keyword='forces', vectorize=True,  approximation="disabled", collater=None):
    out = forward(data)
    ...
    grad_outputs = get_dense_grad_outputs(data, num_rows=num_hessian_rows, density=num_perts_per_row)
...
def get_jacobian_finite_difference(forces, batch, grad_outputs, forward, detach, collater, looped=False, force_keyword='forces', h=0.001):
    ...
    if not looped:
        ...
        perturbed_forces = []
        for p_batch in perturbed_batches:
            pert_force = forward(p_batch)[force_keyword]['forces'].detach() if detach else forward(p_batch)[force_keyword]['forces']
