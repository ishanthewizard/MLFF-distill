...
class TeacherLabelGenerator(Runner):
    ...
    def __init__(
        ...
        # self.normalize_factor = 1.423
        self.normalize_factor = 1.433569
    ...
    def evaluate(self) -> None:
        """
        This function is a placeholder for evaluation logic.
        It can be extended to include evaluation metrics or other operations.
        """
        logging.info("Running evaluation...")
        # Here you can add your evaluation logic
        # For now, we just log the sizes of the dataloaders
        logging.info(f"Train Dataloader size {len(self.train_dataloader)}")
        logging.info(f"Eval Dataloader size {len(self.eval_dataloader)}")
        model = self.train_eval_unit.model
        model.eval()  # Set the model to evaluation mode
        # per atom forces mae
        total_mae = []
        for i, batch in tqdm(enumerate(self.eval_dataloader), total=len(self.eval_dataloader)):
            batch = batch.to(self.device)
            with torch.no_grad():
                output = model(batch)
            forces = output['forces']['forces']
            mae = (forces - (batch.forces / self.normalize_factor)).abs().mean()
            total_mae.append(mae.detach().cpu().item())
        logging.info(f"Total MAE for evaluation dataset: {sum(total_mae) / len(total_mae)}")
