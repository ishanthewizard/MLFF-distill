...
class HessianModelWrapper(HydraModel):
    def get_sampled_hessian(self, data, out):
        ...
        jac = get_jacobian(forces, data.pos, grad_outputs, create_graph=True, looped=False) # num_samples, num_atoms, 
        # jac = get_jacobian_finite_difference(forces, data, grad_outputs, super().forward, detach=False, collater=None, looped=True, h= 0.0001)
