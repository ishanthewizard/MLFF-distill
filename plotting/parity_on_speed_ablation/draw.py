#!/usr/bin/env python3
"""Parity plots: student variants and OPLS vs experiment."""
...
CSV_PATH = SCRIPT_DIR / "main - student_performance-speed.csv"
OUT_DIR = SCRIPT_DIR / "output"
OUT_DIR.mkdir(exist_ok=True)

# Groups: all methods vs experiment
METHODS = [
    "50ps student",
    "100ps student",
    "micro student",
...
# Axis display labels (one per method, order matches METHODS)
X_AXIS_LABEL = "experiment"
Y_AXIS_LABELS = [
    "293K 50ps student original",
    "293K 100ps student",
    "293K 100ps micro student",
    "OPLS",
]

# Column mapping: group short name -> full CSV column name (per species)
COL_MAP = {
    "cation": {
        "50ps student":            "293K 50ps student cation diffusivity",
        "100ps student":           "293K 100ps student cation diffusivity(17ns)",
        "micro student":           "293K 100ps micro student cation diffusivity",
    ...
    "anion": {
        ...
        "micro student":           "293K 100ps micro student anion diffusivity",
    ...
    "solvent": {
        ...
        "micro student":           "293K 100ps micro student solvent diffusivity",
...
def plot_pairwise_parity(ax, x, y, xlabel, ylabel, colors=None):
    """Scatter plot with Pearson r and parity line."""
    ...
    if colors is not None:
        ...
        for sp, col in [("cation", "C0"), ("anion", "C1"), ("solvent", "C2")]:
            m = c == sp
            if m.any():
                ax.scatter(x_clean[m], y_clean[m], s=60, alpha=0.8, c=col, label=sp,
...
def main():
    ...
    # Merge cation, anion, solvent into one dataset (same order for all groups)
    data = {}
    for g in METHODS + ["exp"]:
        vals = []
        for species in ["cation", "anion", "solvent"]:
            col = COL_MAP[species][g]
            v = pd.to_numeric(df[col], errors="coerce")
            vals.extend(v.tolist())
        data[g] = np.array(vals)
    colors = np.array([sp for sp in ["cation", "anion", "solvent"] for _ in range(len(df))])

    # 6 plots: each method vs experiment, arranged in 2 rows × 3 cols
    ...
    out_path = OUT_DIR / "parity_merged.png"
