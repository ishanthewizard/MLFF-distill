...
from typing import Annotated, Generic, Literal, TypeAlias, cast
...
class Output(Base[PretrainConfig], nn.Module):
    ...
    def forward(self, data: BaseData, backbone_out: GOCBackboneOutput):
        ...
        final_node_embeddings = backbone_out["final_node_embeddings"]

        return E, F, final_node_embeddings
...
class PretrainModel(LightningModuleBase[TConfig], Generic[TConfig]):
    ...
    # Update the forward method of the PretrainModel to recompute the edge distances and vectors.
    @override
    def forward(self, batch: BaseData):
        from jmp.utils.goc_graph import GRAPH_TYPES

        # Recompute the edge_distances to include `pos` in the computation graph.
        for graph_type in GRAPH_TYPES:
            edge_index = getattr(batch, f"{graph_type}_edge_index")
            j, i = edge_index
            ...
            # See goc_graph.py, line 304
            edge_vector = -distance_vec / edge_dist[:, None]

            setattr(batch, f"{graph_type}_distance", edge_dist)
            setattr(batch, f"{graph_type}_vector", edge_vector)
