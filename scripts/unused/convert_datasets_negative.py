...
def record_and_save(dataset, file_path, fn, fjacs=False):
    ...
    with env.begin(write=True) as txn:
        ...
        for sample in dataset:
            ...
            sample_output = fn(
                sample
            )  # this function needs to output an array where each element correponds to the label for an entire molecule
...
def convert_labels(labels_folder, new_labels_folder, model="medium"):
    ...
    teacher_force_train_dataset = SimpleDataset(
        os.path.join(labels_folder, "train_forces")
    )
    ...
    record_and_save(
        force_jac_dataset,
        os.path.join(new_labels_folder, "force_jacobians", "force_jacobians.lmdb"),
        multiply_std,
        fjacs=True,
    )
    record_and_save(
        teacher_force_train_dataset,
        os.path.join(new_labels_folder, "train_forces", "train_forces.lmdb"),
        multiply_std,
    )
    record_and_save(
        teacher_force_val_dataset,
        os.path.join(new_labels_folder, "val_forces", "val_forces.lmdb"),
        multiply_std,
    )
...
if __name__ == "__main__":
    ...
    new_labels_folder = (
        "/data/ishan-amin/hessian_proj_data/labels/SPICE_labels/EScAIP_SpiceMonomers"
    )
