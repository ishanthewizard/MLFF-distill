...
def record_labels(labels_folder, dataset_path, model="large"):
    ...
    val_dataset = registry.get_dataset_class("lmdb")(
        {"src": os.path.join(dataset_path, "val")}
    )

    # Load the model
    calc = mace_off(
        model=model, dispersion=False, default_dtype="float32", device="cuda"
    )
    ...
    def get_hessians(sample):
        ...
        return -1 * hessian.reshape(
            natoms, 3, natoms, 3
        )  # this is SUPER IMPORTANT!!! multiply by -1

    record_and_save(
        train_dataset,
        os.path.join(labels_folder, "force_jacobians", "force_jacobians.lmdb"),
        get_hessians,
    )
    record_and_save(
        train_dataset,
        os.path.join(labels_folder, "train_forces", "train_forces.lmdb"),
        get_forces,
    )
    record_and_save(
        val_dataset,
        os.path.join(labels_folder, "val_forces", "val_forces.lmdb"),
        get_forces,
    )
