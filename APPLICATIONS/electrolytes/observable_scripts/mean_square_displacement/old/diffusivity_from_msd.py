...
dt_fallback  = 0.05    # ps (100 fs) if frame.info['time'] is absent
...
for idx, (traj_path, solv) in enumerate(traj_info):
    ...
    # unload msd_dict
    with open(out_dir / f"msd_dict_{solv}.pkl", "rb") as f:
        msd_dict = pickle.load(f)
    msd_na = msd_dict["msd_na"]
    se_na = msd_dict["se_na"]
    msd_p = msd_dict["msd_p"]
    se_p = msd_dict["se_p"]
    tau = msd_dict["tau"]
    dt_ps = msd_dict["dt_ps"]
    EQ_TIME_PS = msd_dict["EQ_TIME_PS"]
    ...
    rows.append({
        "solvent": solv,
        "frames_total": msd_dict["frames_total"],
