#!/usr/bin/env python3
"""Force MAE computation between teacher and student models on a trajectory.
...
def compute_force_mae(
    ...
    """Compute per-frame force MAE between teacher and student on a trajectory.

    If student_ckpt is None, forces stored in the trajectory's calc results
    are used as the student reference (avoids redundant re-inference).
    ...
    Returns dict with keys:
        ...
        mae       : ndarray (n,)  — mean |F_teacher − F_student| per frame (eV/Å)
        max_err   : ndarray (n,)  — max component error per frame (eV/Å)
        rmse      : ndarray (n,)  — RMS force error per frame (eV/Å)
    ...
    times, mae_list, max_err_list, rmse_list = [], [], [], []
    with Trajectory(str(traj_path), mode="r") as traj:
        for idx in tqdm(indices, desc=f"force_mae {traj_path.name}", unit="frame"):
            at = traj[idx]
            at.set_pbc([True, True, True])
            at.wrap()
            # omol task requires charge and spin in atoms.info; set neutral
            # closed-shell defaults so FAIRChem does not warn on every frame.
            at.info.setdefault("charge", 0)
            at.info.setdefault("spin", 1)

            at_t = at.copy()
            at_t.calc = teacher_calc
            f_teacher = at_t.get_forces()  # (N, 3) eV/Å

            if student_calc is not None:
                at_s = at.copy()
                at_s.calc = student_calc
                f_student = at_s.get_forces()
            elif at.calc is not None and "forces" in at.calc.results:
                f_student = np.array(at.calc.results["forces"])
            else:
                continue

            err = np.abs(f_teacher - f_student)          # (N, 3)
