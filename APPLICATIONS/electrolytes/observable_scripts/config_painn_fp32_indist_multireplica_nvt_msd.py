"""Config: multi-replica MSD for the PAINN FP32 In-distribution NVT set.

NVT-langevin analogue of `config_painn_fp32_indist_multireplica_msd.py`.
Computes MSD (-> diffusivity / Nernst-Einstein conductivity) for the 4 NVT
replicas (nvt_replica_0..3) of every electrolyte system, overlaying the
...
composition/box/temperature. Note replica_0 is missing 3 runs
(naotf_pc/1M, naotf_tgdme/1M, napf6_tgdme/*), so those systems get 3 replicas.

All NVT runs in this set are the full 20 ns (uniform file sizes across replicas),
so — unlike the NPT config — no per-system SHORT window capping is needed.
...
Run with:
  /global/homes/y/yuejian/project/MLFF-distill/yuejian/envs/fairchemV2_new/bin/python \
    eval.py --config config_painn_fp32_indist_multireplica_nvt_msd.py
"""
import re
