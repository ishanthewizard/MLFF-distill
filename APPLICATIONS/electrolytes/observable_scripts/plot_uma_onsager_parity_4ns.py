#!/usr/bin/env python3
"""Parity plot: simulated byteff2 Onsager ionic conductivity at the 0-4 ns window
vs experimental conductivity, for the UMA 1 M / 298 K electrolytes.

Reads conductivity_final_window_vs_exp.csv. Log-log, identical x/y limits + equal
aspect so the 1:1 line is a true diagonal. Solvent -> colour, salt -> marker.
"""
...
META = {
    "napf6_diglyme": ("PF6", "Diglyme"), "napf6_pc": ("PF6", "PC"),
    "naotf_diglyme": ("OTf", "Diglyme"), "naotf_dme": ("OTf", "DME"),
    "napf6_dme": ("PF6", "DME"),
}
...
df = df.dropna(subset=["exp_sigma_mS_cm", "sigma_onsager_mS_cm"])

x = df["exp_sigma_mS_cm"].to_numpy(float)
y = df["sigma_onsager_mS_cm"].to_numpy(float)
...
for _, r in df.iterrows():
    ...
    ax.scatter(r["exp_sigma_mS_cm"], r["sigma_onsager_mS_cm"],
               marker=SALT_MARKER[salt], s=130, color=SOLVENT_COLOR[solv],
               edgecolor="k", linewidth=0.8, zorder=3)
    ax.annotate(r["system"], (r["exp_sigma_mS_cm"], r["sigma_onsager_mS_cm"]),
...
ax.set_ylabel("Simulated Onsager σ @ 0–4 ns (mS/cm)")
ax.set_title("UMA 1 M 298 K — Onsager conductivity parity (0–4 ns)\n"
...
fig.tight_layout()
p = COND_OUT / "parity_conductivity_Onsager_4ns.png"
fig.savefig(p, dpi=150, bbox_inches="tight")
plt.close(fig)
print("wrote", p)
print(df[["system", "exp_sigma_mS_cm", "sigma_onsager_mS_cm"]].to_string(index=False))
