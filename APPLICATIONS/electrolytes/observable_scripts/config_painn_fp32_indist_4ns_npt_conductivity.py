"""Config: 4 ns-window CONDUCTIVITY for the PAINN FP32 In-distribution NPT set.

Same trajectory set / auto-discovery as
``config_painn_fp32_indist_multireplica_conductivity.py``, but restricted to the
FIRST 4 ns of usable data for a convergence / trustability check ("does 4 ns of
sampling already give the conductivity?").
...
(trajectories are the full 20 ns / 200k frames @ 100 fs; the .fennol.yaml `nsteps`
and the dirname labels `1ns_1ps` / `2ns_100fs` are BOTH unreliable -- verified
len(traj)=200000. eval.py caps max_traj_ns to the real length, so 6.0 is exact.)

Frame budget at these settings (dt_ps=0.1, stride=round(5/0.1)=50):
  n_loaded = (6e3/0.1 - 2e3/0.1) // 50 = (60000 - 20000)//50 = 800 frames
  byteff2 needs n_loaded > nt_end(200) + 200 = 400  -> 800 OK
  mdcraft collective fits 0.3-2.0 ns lags -> comfortably inside the 4 ns window.
...
NOTE on load_dt_ps=5.0 and the byteff2 fit window:
  ...
  mdcraft collective-Onsager backend instead fits 0.3-2.0 ns (independent of load_dt).

Run with (multi-hour job: 72 trajectories, 800 frames x 2 backends each):
  /global/homes/y/yuejian/project/MLFF-distill/yuejian/envs/fairchemV2_new/bin/python \
    eval.py --config config_painn_fp32_indist_4ns_npt_conductivity.py
"""
import re
...
OUTPUT_DIR = (
    "/global/homes/y/yuejian/project/MLFF-distill"
    "/m5250/distillation_proj/simulation_results/PAINN/electrolytes_data"
    "/analysis/4ns_conductivity_check/npt"
)
