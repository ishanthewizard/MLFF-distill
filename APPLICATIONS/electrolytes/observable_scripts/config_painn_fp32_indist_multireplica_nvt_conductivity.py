...
NVT-langevin analogue of `config_painn_fp32_indist_multireplica_conductivity.py`.
Mirrors the NVT MSD config (same trajectory set / auto-discovery), but runs the
...
writes one conductivity.csv row for) every replica of every system. replica_0 is
missing 3 runs (naotf_pc/1M, naotf_tgdme/1M, napf6_tgdme/*), so those systems get
3 replicas.
...
Conductivity windowing (per user request):
  ...
  - to the end (20 ns target)        -> max_traj_ns = 20.0  (eval.py caps each
                                        replica to its actual length; all full 20 ns)
...
Run with (multi-hour job: ~70 trajectories, ~18 ns each, two backends):
  /global/homes/y/yuejian/project/MLFF-distill/yuejian/envs/fairchemV2_new/bin/python \
    eval.py --config config_painn_fp32_indist_multireplica_nvt_conductivity.py
"""
import re
