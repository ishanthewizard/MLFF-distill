"""Config: multi-replica CONDUCTIVITY for the PAINN FP32 In-distribution NPT set.

Mirrors `config_painn_fp32_indist_multireplica_msd.py` (same trajectory set /
auto-discovery), but runs the conductivity analysis instead of MSD.
...
writes one conductivity.csv row for) every replica of every system.
...
Conductivity windowing (per user request):
  ...
  - effective sampling / lag dt 1 ps -> conductivity_load_dt_ps = 1.0
...
NOTE on load_dt_ps=1.0 and the byteff2 fit window:
  byteff2's onsager_calc() hardcodes its MSD-slope fit to lags [50, 200) frames
  (after dropping the first 200 loaded frames). With load_dt_ps=1.0 ps that is a
  50-200 ps lag window for the Nernst-Einstein / byteff2-Onsager numbers. The
  mdcraft collective-Onsager backend instead fits 0.3-2.0 ns (independent of
  load_dt) and a 1 ps VACF is ideal for it, so the collective conductivity is the
  reliable number at this dt.
...
Run with (multi-hour job: 72 trajectories, ~18k frames each, two backends):
  /global/homes/y/yuejian/project/MLFF-distill/yuejian/envs/fairchemV2_new/bin/python \
    eval.py --config config_painn_fp32_indist_multireplica_conductivity.py
"""
import re
...
LOAD_DT_PS  = 5.0      # effective sampling / MSD-lag dt = 1 ps
