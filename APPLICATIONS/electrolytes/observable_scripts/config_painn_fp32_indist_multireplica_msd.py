"""Config: multi-replica MSD for the PAINN FP32 In-distribution NPT set.

Computes MSD (-> diffusivity / Nernst-Einstein conductivity) for the 4 NPT
replicas (npt_replica_0..3) of every electrolyte system, overlaying the
...
nsteps=20e6; the dirname labels `1ns_1ps` / `2ns_100fs` are NOT reliable.
eval.py caps max_traj_ns to the actual trajectory length.)

cat/anion/solvent symbols are inferred from the `<salt>_<solvent>` dir via the
lookup tables below (required by the msd analysis).

Run with:
  /global/homes/y/yuejian/project/MLFF-distill/yuejian/envs/fairchemV2_new/bin/python \
    eval.py --config config_painn_fp32_indist_multireplica_msd.py
"""
import re
...
# These systems have a short replica_3 (18.5-19.75 ns) while replicas 0-2 reach
# the full 20 ns. Cap the whole system to a common 18.4 ns window (just below
# the shortest run, naotf_tgdme/npt_0_1M r3 = 18.50 ns) so all 4 replicas are
# compared over the same span. eval.py caps each model to min(max_traj_ns,
# actual length), so the longer replicas are truncated to 18.4 ns too.
SHORT_MAX_TRAJ_NS = 18.4
SHORT_SYSTEMS = {
    "naotf_tgdme__npt_0_1M_298K_2ns_100fs",
    "naotf_tgdme__npt_1M_298K_2ns_100fs",
    "napf6_tgdme__npt_1M_298K_2ns_100fs",
    "naotf_diglyme__npt_1M_298K_2ns_100fs",
    "napf6_diglyme__npt_1M_298K_2ns_100fs",
}
...
# after eq_cut. avail differs between the 20 ns and 18.4 ns systems, so a single
# fixed n_frames would NOT give a fixed dt. Setting
#   n_frames = usable_ns * 1000 / TARGET_MSD_DT_PS
# per system pins the lag step to exactly TARGET_MSD_DT_PS everywhere (instead
# of the default n_frames=1000, which gives ~18 ps for 20 ns vs ~16.4 ps for
# the 18.4 ns runs).
TARGET_MSD_DT_PS = 10.0   # ps
...
for key in sorted(_systems):
    ...
    max_traj_ns = SHORT_MAX_TRAJ_NS if name in SHORT_SYSTEMS else MAX_TRAJ_NS
