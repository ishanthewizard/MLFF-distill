"""Config: multi-replica MSD + conductivity + cell size + potential energy for
the UMA NPT "concentrate" set (UMA_multi_replicas/npt).

Layout (differs from the PAINN scheme — ONE dir per system per replica, the traj
is `<dir>.traj` inside it):

  npt/UMA_ref_concentrate_rep{1..5}/<salt>_<solvent>_npt_<conc>M_<temp>K/
        └── <salt>_<solvent>_npt_<conc>M_<temp>K.traj

The 5 replicas (rep1..rep5) become the "models" (`replica_1..replica_5`) of each
unique system `<salt>_<solvent>_npt_<conc>M_<temp>K`, so all 5 replicas of a
system are overlaid on the same axes / land in the same combined CSV. 11 systems
× 5 replicas = 55 trajectories.

Trajectory params (from the .log — integrator dt = 1 fs, 10 steps/saved-frame):
  - dt_fs        = 10.0     (user: 10 fs/saved-frame)
  - max_traj_ns  = 1.0      (user: "analyze for 1 ns"; every run is ~1.1-1.35 ns,
                             so eval.py uses frames [eq_cut, 1] ns; it caps
                             max_traj_ns to the real length so 1.0 is safe)

MSD / conductivity windowing (user choices):
  - eq_cut_ns          = 0.05    (discard first 0.05 ns as equilibration)
  ...
  - MSD n_frames pinned so the subsampled lag step == 0.5 ps:
        n_frames = (max_traj_ns - eq_cut_ns) * 1000 / TARGET_MSD_DT_PS
                 = (1.0 - 0.05) * 1000 / 0.5 = 1900
  - conductivity_load_dt_ps = 0.5 -> ~1900 loaded frames from the 1 ns window.
    byteff2 onsager_calc() drops the first 200 loaded frames then fits MSD-slope
    lags [50, 200); 1900 >> 400 so both the byteff2 Onsager and Nernst-Einstein
    numbers are well defined (fit window ~25-100 ps of lag). The mdcraft
    collective-Onsager backend fits its own 0.3-2.0 ns window.

cat/anion/solvent symbols + concentration + temperature are parsed from the dir
name via the lookup tables below (required by msd/conductivity, and by the
group merge exp-matcher).

NOTE (cost): MSD is a serial O(n_frames^2) python loop and these CFS trajectories
are slow random-access reads (~0.4 s/frame). With n_frames=1900 and 55 trajs this
is a multi-hour job — run on a SLURM CPU node, NOT a login node
(see submit_uma_multireplica_npt.sh).

Run with:
  /global/homes/y/yuejian/project/MLFF-distill/yuejian/envs/fairchemV2_new/bin/python \
    eval.py --config config_uma_multireplica_npt.py
"""
import re
...
_NPT_ROOT = Path(
    ...
    "/m5250/distillation_proj/simulation_results/UMA/UMA_multi_replicas/npt"
)
REPLICA_DIRS = {
...
MAX_TRAJ_NS = 1.0      # analyze the first 1 ns; eval.py caps to actual length

# ── MSD / conductivity windowing (user choices) ────────────────────────────────
EQ_CUT_NS        = 0.05    # discard first 0.05 ns
...
# MSD linear-fit / convergence knobs, scaled for a ~0.95 ns usable window
...
for name in sorted(_systems):
    ...
    SYSTEMS.append({
        ...
        "n_frames":        n_frames,           # MSD subsample + energy timeseries
        ...
        # ── potential-energy timeseries / cell size ──
        "skip_ns":                 EQ_CUT_NS,  # equilibration ref line
        "cell_size_analyze_dt_ps": 5.0,        # NPT box: sample every 5 ps (~190 pts)
    })

OUTPUT_DIR = (
    "/global/homes/y/yuejian/project/MLFF-distill"
    "/m5250/distillation_proj/simulation_results/UMA/UMA_multi_replicas/analysis"
)
ANALYSES = ["msd", "conductivity", "cell_size", "energy"]
