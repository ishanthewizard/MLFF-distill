...
traj_infos = [
    ("/u/czhang31/MLFF-distill/APPLICATIONS/electrolytes/md_results/napf6_s1p1_distilled_ishan_ckpt_again.traj","s1p1"),
    ("/u/czhang31/MLFF-distill/APPLICATIONS/electrolytes/md_results/natfsi_122k_1ns.traj","s1p1"),
]

traj_info = [traj_infos[0]] if salt == 'napf6' else [traj_infos[1]]

# Read last N frames (skip the final 100 if you like; change as needed)
read_slice = "-10000:"

# Fallback frame time step if frame.info['time'] missing (ps)
dt_fallback = 0.1
...
# ───────────────────── main loop ─────────────────────────────────────────────
rows = []
out_dir = Path("/u/czhang31/MLFF-distill/APPLICATIONS/electrolytes/msd_plots")
...
for traj_path, system in traj_info:
    ...
    p_idx  = [i for i, s in enumerate(symbols0) if s == element]  # PF6 tracer via P

    # unwrap trajectories
    print(f"{system}: unwrapping Na ...")
    pos_na = unwrap_positions(frames, na_idx)  # (T, Nna, 3)
    print(f"{system}: unwrapping {element} ...")
    ...
    print(f"[{system}] D({element})  = {D_p_A2_ps:.4f} Å²/ps  → {D_p:.4e} cm²/s\n")
    ...
    plt.plot(tau, msd_p,  lw=1.8, label=f"{element} MSD")
    ...
    plt.plot(tau[start:], fit_p[start:],  linestyle="--", lw=1.2, label=f"{element} fit")

    plt.xlabel("τ (ps)")
    plt.ylabel("MSD (Å$^2$)")
    plt.title(f"MSD: {salt} DME distilled")
    plt.legend(frameon=False)
    plt.tight_layout()
    out_png = out_dir / f"msd_{system}_{salt}_{element}.png"
