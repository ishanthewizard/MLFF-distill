...
if __name__ == "__main__":
    train_path = '/global/homes/y/yuejian/project/MLFF-distill/m4558/distillation_project/toy_test/Sep_18_naotf_per_salt_data/train'
    force_rms, linref_coeff = compute_normalizer_and_linear_reference(
        train_path, 8
    )
    
    print(f"Force RMS: {force_rms}")
    print(f"Linear reference coefficients: {linref_coeff}")
    ...
    yaml_data = {
        "omol_element_refs": linref_coeff
    }
    ...
    print(f"Linear reference coefficients saved to: {output_file}")
    
    # Also save force RMS to a separate file for reference
    rms_file = parent_dir / "force_rms.txt"
    with open(rms_file, 'w') as f:
        f.write(f"{force_rms}\n")
    
    print(f"Force RMS saved to: {rms_file}")