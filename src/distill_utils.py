...
def get_teacher_jacobian(forces, batch, vectorize=True, should_mask=True):
    ...
    grad_outputs = torch.zeros((max_free_atom_per_mol, 3, total_num_atoms, 3)).to(
        forces.device
    )
    for i, free_atoms_in_mol in enumerate(num_free_atoms_per_mol):
        ...
        grad_outputs[indices, :, offset_indices, :] = torch.eye(3)[None, :, :].to(
            forces.device
        )
    jac = get_jacobian(
        forces, batch.pos, grad_outputs, looped=(not vectorize)
    )  # outputs a max_free_atom_per_mol x 3 x total_num_atoms x 3 matrix.

    jacs_per_mol = [
        jac[:n_fr_at, :, cum_sum : cum_sum + nat, :]
        for cum_sum, n_fr_at, nat in zip(
            cumulative_sums[:-1], num_free_atoms_per_mol, natoms
        )
    ]
...
def get_force_jac_loss(
    ...
    for i, atoms_in_mol in enumerate(batch.natoms):
        ...
        grad_outputs[
            torch.arange(samples.shape[0]), offset_samples[:, 0], offset_samples[:, 1]
        ] = 1
    ...
    jacs_per_mol = [
        ...
        for mol_samps, cum_sum, nat in zip(by_molecule, cumulative_sums[:-1], natoms)
    ]
    ...
    for i, samples in enumerate(by_molecule):
        fixed_atoms = ~mask[cumulative_sums[i] : cumulative_sums[i + 1]]
        fixed_cumsum = torch.cumsum(fixed_atoms, dim=0)
        curr = batch["force_jacs"][cum_jac_indexes[i] : cum_jac_indexes[i + 1]].reshape(
        ...
        #     keep = torch.where(samples[:, 0] == valid_rows)[0]
        #     subsampled_curr = curr[keep.long(), samples[:, 1]]  # get the sampled rows
...
def get_energy_jac_loss(out, batch, energy_std):
    ...
    neg_force_label_normed = (
        neg_force_label / energy_std
    )  # we've been training our energies to normalized targets (labels /= std), so we need to do the same here

    energy_jac = torch.autograd.grad(
        out["energy"].sum(), batch.pos, create_graph=True, retain_graph=True
    )[0]

    custom_loss = lambda jac, true_jac: torch.norm(jac - true_jac, p=2, dim=-1).sum(
        dim=0
    )
    ...
    loss = loss / sum(
        batch.natoms
    )  # different options here, currently weighting all systems the same regardless of size

    return loss


def get_jacobian_finite_difference(
    forces, batch, grad_outputs, forward, collater, looped=False, h=0.0001
):
    ...
    for i in range(len(perturbed_batches)):
        forward_force = perturbed_forces[
            i * total_num_atoms : (i + 1) * total_num_atoms
        ]
