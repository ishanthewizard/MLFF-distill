...
def get_force_jac_loss_masked(out, batch, num_samples, mask, should_mask, looped=False, finite_differences=False, forward=None, collater=None):
    ...
    losses = []
    # BUG: could be buggy?
    valid_masked_natoms = [torch.tensor(0, device=batch.natoms.device)]
    for i, (jac, true_jac, hessian_mask) in enumerate(zip(jacs_per_mol, true_jacs_per_mol, batch.hessian_mask)):

        loss = custom_loss(jac, true_jac) * hessian_mask
        losses.append(loss)
    ...
    loss = sum(valid_losses)
    # print(num_free_atoms_per_mol)
    # BUG: this might cause bug when moving on to parallel
    num_samples = sum(torch.tensor(valid_masked_natoms,device = batch.natoms.device))
    num_samples = distutils.all_reduce(num_samples, device=forces.device)
    # Multiply by world size since gradients are averaged
    # across DDP replicas
    loss  = loss * distutils.get_world_size() / (num_samples+1e-8)
    # print("FORCE JAC LOSS", loss.item())
    return loss 
