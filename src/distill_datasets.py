...
class CombinedDataset(Dataset):
    def __init__(
        ...
        if force_jac_dataset and len(main_dataset) != len(force_jac_dataset):
            ...
            raise Exception("DIFF SIZE!!")
        if final_node_feature_dataset and len(main_dataset) != len(
    ...
    def __getitem__(self, idx):
        ...
        if self.force_jac_dataset:
            ...
            force_jacs = self.force_jac_dataset[
                idx
            ]  # DON'T RESHAPE! We'll do it later, easier for atoms of different lengths
        else:
            force_jacs = None
        ...
        if not hasattr(main_batch, "fixed"):
            main_batch.fixed = torch.zeros(
                main_batch.pos.shape[0], device=main_batch.pos.device
            )
        main_batch.teacher_forces = teacher_forces
        main_batch.force_jacs = force_jacs
        if final_node_features is not None:
            main_batch.final_node_features = final_node_features.reshape(
                teacher_forces.shape[0], -1
            )
        return main_batch
...
class SimpleDataset(Dataset):
    ...
    def merge_lmdb_files(self, output_file, map_size=1099511627776 * 2):
        ...
        env = lmdb.open(
            os.path.join(output_file, "data.lmdb"), map_size=int(map_size)
        )  # Adjust map_size as needed
