...
class LabelsTrainer(OCPTrainer):
    def __init__(self, *args, **kwargs):
        ...
        # Compute teacher MAE
        self.load_teacher_model_and_record(
            self.config["dataset"]["teacher_labels_folder"]
        )
        sys.exit(0)
    ...
    def load_teacher_model_and_record(self, labels_folder):
        ...
        checkpoint = torch.load(
            self.config["dataset"]["teacher_checkpoint_path"],
        ...
        if self.config["dataset"].get("teacher_scale_file", None):
            self.teacher_config["model_attributes"]["scale_file"] = self.config[
                "dataset"
            ]["teacher_scale_file"]
    ...
    def launch_record_tasks(self, labels_folder, dataset_type):
        ...
        end_idx = (
            start_idx + indices_per_worker if rank < world_size - 1 else dataset_size
        )
    ...
    def record_labels_parallel(self, labels_folder, dataset_type, start_idx, end_idx):
        ...
        lmdb_path = os.path.join(
            labels_folder,
            f"{dataset_type}_forces",
            f"data.{distutils.get_rank():04d}.lmdb",
        )
        ...
        def get_seperated_forces(batch):
            ...
            natoms = batch.natoms
            return [
                all_forces[sum(natoms[:i]) : sum(natoms[: i + 1])]
        ...
        if dataset_type == "train":
            ...
            self.record_and_save(
                jac_dataloader, jac_lmdb_path, get_seperated_force_jacs
            )
            ...
            for batch in tqdm(dataloader):
                with env.begin(write=True) as txn:
                    ...
                    for i in range(len(batch_ids)):
                        txn.put(
                            batch_ids[i].encode(),
                            batch_output[i].detach().cpu().numpy().tobytes(),
                        )
